...
def calculate_risk_score(shared_device_accounts, bonus_claims, unusual_deposits):
    ...
    This is a toy scoring model for learning purposes:
    - Each extra account sharing a device adds risk (multi-accounting signal)
    - Each bonus claim adds a smaller amount of risk (bonus-abuse signal)
    - An unusual deposit pattern adds a flat risk bump
    """
    score = 0

    # Signal 1: multiple accounts on one device
    score += shared_device_accounts * 15

    # Signal 2: bonus claims
    score += bonus_claims * 8

    # Signal 3: unusual deposit pattern
    if unusual_deposits:
        score += 25
